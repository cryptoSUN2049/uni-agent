...
The included `verifier.py` is a deterministic smoke fixture. It checks exact trace and envelope hashes and awards one only when `metadata.expected_response_sha256` matches the response. Every trusted verifier must also emit a boolean top-level `eligible`: a safe, auditable policy failure remains eligible with reward zero, while tampered, stale, or unsafe evidence is ineligible. The Task binds that decision into the receipt and reward lineage, and the required trajectory admission rejects an ineligible episode before TransferQueue. Replace this fixture with a benchmark-specific verifier before any training release; never use a non-fresh or actor-supplied score as online reward.
